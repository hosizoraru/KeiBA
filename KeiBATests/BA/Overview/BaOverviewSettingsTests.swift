...
final class BaOverviewSettingsTests: XCTestCase {
    func testSettingsStoreStartsWithCurrentEnvelopeDefaults() throws {
        let defaults = try makeIsolatedDefaults()
        let base = Date(timeIntervalSince1970: 1_700_000_000)
        let expected = BaSettingsEnvelope.defaults(now: base).normalized()

        let store = BaSettingsStore(defaults: defaults)
        let envelope = store.loadEnvelope()
        let account = try XCTUnwrap(envelope.accounts.first)

        XCTAssertEqual(envelope.schemaVersion, BaSettingsEnvelope.currentSchemaVersion)
        XCTAssertEqual(envelope.selectedServer, expected.selectedServer)
        XCTAssertEqual(envelope.selectedAccountID, BaAccountProfile.defaultID(for: .cn))
        XCTAssertEqual(account.id, BaAccountProfile.defaultID(for: .cn))
        XCTAssertEqual(account.server, .cn)
        XCTAssertEqual(account.profile.nickname, "Kei")
        XCTAssertEqual(account.profile.friendCode, "ARISUKEI")
        XCTAssertEqual(envelope.globalSettings, expected.globalSettings)
        XCTAssertNil(defaults.data(forKey: "ba.app.settings.v2"))
    }
    ...
    private func makeIsolatedDefaults() throws -> UserDefaults {
        let suiteName = "KeiBATests.\(UUID().uuidString)"
